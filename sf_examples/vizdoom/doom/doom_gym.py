...
class VizdoomEnv(gym.Env):
    def __init__(
        ...
        # game ALREADY initialized, before new_episode(). Necessary because, in the
        # eval flow, the env persists between episodes and the Doom engine is NOT re-seeded
        # by reset(seed=) (seed() only updates self.curr_seed, not the engine) -> the
        # base image of reset varied between executions (tested: max|Δ|≈127/255).
        # With forced set_seed, the reset obs becomes bit-identical. Remains None in
        # training -> byte-identical behavior to original.
    ...
    def reset(self, **kwargs) -> Tuple[np.ndarray, Dict]:
        ...
        # EVAL-ONLY: recreate the Doom game with the episode seed, to reset the RNG
        # of the ZDoom simulation (set_seed alone in an already live game is not enough — only
        # init() re-seeds). Guarantees reproducible trajectory between episodes in the same
