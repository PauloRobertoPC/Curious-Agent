...
class TrajectoryVisualizationWrapper(gym.Wrapper):
    def __init__(self, env:gym.Env, image_directory:str,  filename_step:int):
        # env
        self.env = env
        ...
        self.filename_step = filename_step
        self.images_count = filename_step
    ...
    def step(self, action):
        ...
        if terminated or truncated:
            generate_trajectory_map(info["sector_lines"], info["agent_trajectory"], info["medikits"], info["poisons"], f"{self.image_directory}/{self.images_count:010d}")
            self.images_count += self.filename_step
